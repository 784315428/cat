...
$("#findPersonalId").on("click", function () {
...
$("#queryShow button[name='ready']").on("click", function () {
    var id = $("#personalId").val();
    if (id.length != 0) {
...
$("#answerShow button[name='ready']").on("click", function () {
...
$("#answerShow button[name='close']").on("click", function () {
    var param = {key: "READY_FOR_ONE_RESPONSE", value: false};
    socket.sendJson(param);
    $("#answerShow").modal("hide")
});
$("#dialogForOne button[name='close']").on("click", function () {
    closeChannel();
...
var openVideoAudio = function (is_video, is_audio) {
    getUserMedia.call(navigator, {video: is_video, audio: is_audio}, function (localMediaStream) {
